...
func (f *LockerFactory) GetLockName(tableName string) string {
	...
	case "azure_blob":
		...
		if f.dbConnectionString != "" {
			...
			serverName := extractServerNameFromConnectionString(f.dbConnectionString)
			if serverName != "" {
...
// extractServerNameFromConnectionString extracts server name from SQL Server connection string
func extractServerNameFromConnectionString(connectionString string) string {
	// Simple extraction of server name from connection string
	// Look for "Server=" or "server=" and extract value until next semicolon
	parts := strings.Split(connectionString, ";")
	for _, part := range parts {
		part = strings.TrimSpace(part)
		if strings.HasPrefix(strings.ToLower(part), "server=") {
			return strings.TrimPrefix(part, "server=")
		} else if strings.HasPrefix(strings.ToLower(part), "data source=") {
			return strings.TrimPrefix(part, "data source=")
		}
	}
	return ""
}
