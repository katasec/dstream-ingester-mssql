...
func (c *CheckpointManager) InitializeCheckpointTable() error {
	...
	if colCount == 0 {
		...
		log.Printf("Added last_seq column to checkpoints table")
	}

	log.Printf("Initialized checkpoints table")
	return nil
}
...
func (c *CheckpointManager) LoadLastLSN() ([]byte, []byte, error) {
	...
	if err == sql.ErrNoRows {
		...
		log.Printf("No previous LSN/SEQ, initializing with defaults for table %s: lsn=%s seq=%s", c.tableName, hex.EncodeToString(lastLSN), hex.EncodeToString(lastSeq))
	} else if err != nil {
		...
		if lastSeq == nil {
			...
			if cdcErr == nil && seqVal != nil {
				lastSeq = seqVal
				log.Printf("Loaded last_seq from CDC change table for table %s: seq=%s", c.tableName, hex.EncodeToString(lastSeq))
			} else {
				startSeqBytes, _ := hex.DecodeString(defaultStartSEQ)
				lastSeq = startSeqBytes
				log.Printf("No last_seq found, initializing with default for table %s: seq=%s", c.tableName, hex.EncodeToString(lastSeq))
			}
		}
	}

	log.Printf("Resuming from last LSN for table %s: lsn=%s seq=%s", c.tableName, hex.EncodeToString(lastLSN), hex.EncodeToString(lastSeq))
...
func (c *CheckpointManager) SaveLastLSN(newLSN []byte, lastSeq []byte) error {
	...
	log.Printf("Saved new LSN and Seq for table %s: lsn=%s seq=%s", c.tableName, hex.EncodeToString(newLSN), hex.EncodeToString(lastSeq))
	return nil
}

// GetLastLSN returns the last LSN as a hex-encoded string for convenience
func (c *CheckpointManager) GetLastLSN() (string, error) {
	lsnBytes, _, err := c.LoadLastLSN()
	if err != nil {
		return "", err
	}
	return hex.EncodeToString(lsnBytes), nil
}
